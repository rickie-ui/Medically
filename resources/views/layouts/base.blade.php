...
<body class="antialiased bg-[#F5F5FD] max-lg:hidden">
    ...
    <aside class="bg-white w-2/12 h-auto border-0 border-r-2">
      ...
      <div class="bg-[#ECFDF9] px-2 py-2 h-20  flex  space-x-4 items-center">
        ...
        <h3 class="text-md opacity-50 font-semibold"> <a href="/patient/{{Auth::guard('patient')->user()->id}}/profile" class="hover:underline">{{Auth::guard('patient')->user()->fullName  ?? ''}}</a><br><span class="text-xs">{{Auth::guard('patient')->user()->email ?? ''}}</span></h3>
      ...
      <ul class="opacity-70">
        <li class="text-md duration-300 hover:text-white hover:bg-[#40C68C] hover:rounded-sm">
          <a href="{{route('patientdash')}}" class="block px-4 py-2 {{ request()->segment(2) == 'dashboard' ? 'bg-[#40C68C] text-white' : '' }}"><i class="fa fa-dashboard"></i>&ensp; Dashboard</a>
        </li>
        <li class="text-md duration-300 hover:text-white hover:bg-[#40C68C] hover:rounded-sm">
          <a href="{{route('appoint')}}" class="block px-4 py-2 {{ request()->segment(2) == 'appointments' ? 'bg-[#40C68C] text-white' : '' }}"><i class="fa fa-briefcase"></i>&ensp; Appointments</a>
        </li>
